...
async getApps() {
    try {
        ...
        console.log('Raw apps data:', apps);
        
        // Don't try to get deployments for each app - just return the apps
        // The port information should come from the app itself or its current deployment
        return apps;
    } catch (error) {
...
getPollingStatus(appId) {
    // This is a placeholder - you'll need to implement actual polling status
    return {
        isPolling: false,
        lastCommit: null,
        interval: null
    ...
    // Enhanced app card with auto-deploy toggle
renderAppsList(apps) {
    return `
        <div class="grid grid-cols-2">
            ${apps.map(app => {
                // Determine the URL to use
                const displayUrl = app.publicUrl || (app.currentPort ? `http://localhost:${app.currentPort}` : 'http://localhost:3004');
                ...
                return `
                    <div class="card app-card">
                            ...
                            <div>
                                ...
                                ${app.autoDeploy ? `
                                    <div style="display:flex;align-items:center;gap:0.5rem;margin:0.5rem 0;">
                                        ...
                                        <small style="color:var(--success);">Auto-deploy enabled (polling every 30s)</small>
                                    </div>
                                ` : ''}
                            </div>
                        ...
                        <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                            <!-- Manual deploy with commit selection -->
                            ...
                            <!-- Auto-deploy toggle -->
                            ${!app.autoDeploy ? `
                                <button class="btn btn-success" onclick="zdh.enableAutoDeploy('${app._id}')">
                                    🔄 Enable Auto-Deploy
                                </button>
                            ` : `
                                <button class="btn btn-warning" onclick="zdh.disableAutoDeploy('${app._id}')">
                                    🛑 Disable Auto-Deploy
                                </button>
                            `}
                            
                            <!-- ALWAYS VISIBLE View App Button -->
                            ${app.currentPort || app.publicUrl ? `
...
// Enable auto-deploy
async enableAutoDeploy(appId) {
  try {
    const result = await this.apiCall(`/apps/${appId}/enable-auto-deploy`, {
      method: 'POST'
    });
    
    if (result.success) {
      this.showNotification('✅ Auto-deploy enabled! Polling for new commits every 30 seconds.', 'success');
      this.loadApps(); // Refresh the app list
    }
  } catch (error) {
    this.showNotification('Failed to enable auto-deploy: ' + error.message, 'error');
  }
}

// Disable auto-deploy
async disableAutoDeploy(appId) {
  try {
    const result = await this.apiCall(`/apps/${appId}/disable-auto-deploy`, {
      method: 'POST'
    });
    
    if (result.success) {
      this.showNotification('🛑 Auto-deploy disabled.', 'success');
      this.loadApps(); // Refresh the app list
    }
  } catch (error) {
    this.showNotification('Failed to disable auto-deploy: ' + error.message, 'error');
  }
}
